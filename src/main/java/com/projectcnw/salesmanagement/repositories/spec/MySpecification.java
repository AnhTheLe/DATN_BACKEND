...
public class MySpecification<T> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        ...
        for (SearchCriteria criteria : list) {
            ...
            } else if (criteria.getOperation().equals(SearchOperation.DATE_START)) {
                LocalDate date = LocalDate.parse(criteria.getValue().toString());
                Timestamp timestamp = Timestamp.valueOf(date.atStartOfDay());
                predicates.add(builder.greaterThanOrEqualTo(root.get(criteria.getKey()), timestamp));
            } else if (criteria.getOperation().equals(SearchOperation.DATE_END)) {
                LocalDate date = LocalDate.parse(criteria.getValue().toString());
                Timestamp timestamp = Timestamp.valueOf(date.atStartOfDay());
